...
def update_labels(request):
    ...
    if request.user.is_authenticated:
        ...
        label_json.append({"data": post_data, "user": context["user"],
            "datetime": datetime.datetime.now().isoformat()})
        tweet.update(labels_automatic=json.dumps(label_json),
                labels_manual=json.dumps(post_data), updated_at=datetime.datetime.now())
...
def manual_remove_labels(request):
    ...
    for item in queryset.iterator():
        d = datetime.datetime.strptime(json.loads(item.labels_automatic)[0]["datetime"], "%Y-%m-%dT%H:%M:%S.%f")
        if start_date < d < end_date:
            tweet = Tweet.objects.filter(id=item.id)
            new_labels = get_label_json(tweet.first().labels_manual)
            if new_labels.get("labels"):
                s = set(["Not Incident Related", "Incident Related", "Corona", "Riot", "Violence"])
                ...
                label_json = get_label_json(tweet.first().labels_automatic)
                label_json.append({"data": new_labels, "user": "SYSTEM", \
                        "datetime": datetime.datetime.now().isoformat()})
                tweet.update(labels_automatic=json.dumps(label_json), labels_manual=json.dumps(new_labels), updateed_at=datetime.datetime.now())
    ...
    for row in csv_reader:
        if line_count == 0:
            ...
            for i, v in enumerate(row):
                tweet_dict[tweet_headers[i]] = v
            save_tweet(
                    url = "https://twitter.com/margabult/status/" + (tweet_dict['id_str']),
...
def save_tweet(full_text, url=None, labels_manual="[]", labels_automatic="[]",
        tweet_group=None, keywords=None, content=None, id_str=None, status="initial"):
    """
    ...
    return Tweet.objects.create(url=url, full_text=full_text,
            labels_manual=labels_manual, labels_automatic=labels_automatic,
            tweet_group=tweet_group, keywords=keywords, content=content, id_str=id_str, status="full_text, active")
